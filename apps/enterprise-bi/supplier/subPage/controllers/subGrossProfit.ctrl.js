class SubGrossProfitController {
  constructor(CommonCon, toolService, SaleStockSubMenu, popups, dataService, popupDataService,
              Table, Chart, basicService, $stateParams, $scope, $rootScope, Pop, Common, CommSearchSort) {
    this.Pop = Pop;
    this.Table = Table;
    this.Chart = Chart;
    this.scope = $scope;
    ...
    this.key = {
      ...
      pageId: CommonCon.page.page_subSupplier_profit,
    ...
    // 保存指标的local
    this.localTable = CommonCon.local.TABLE_ORIGIN_SUP_GROSS_PROFIT;
    this.localChart = CommonCon.local.CHART_DATA_SUP_GROSS_PROFIT;
  }
   ...
   */
  getTableOption() {
    const promise = this.popups.popupGrossProfitTable({field: this.currFileds});
...
angular.module("hs.supplier.saleStock").controller("subGrossProfitCtrl", SubGrossProfitController);
