class supplyItemController {
  constructor(DTColumnBuilder, $scope, $compile, tableService, CommonCon, alert, Symbols,
              toolService, Field, basicService, Pop, $state, $location, Common, indexCompleteService, dataService) {
    this.Pop = Pop;
    this.alert = alert;
    this.Common = Common;
    this.Field = Field;
  ...
  init() {
    //当前页面基准日期 | 财务月 获取
    this.indexCompleteService.getBasicDate(this);
  ...
  initColumn(isChange,n_param) {
    this.tool.changeCol(this.field, null, this.keys, f => {
      if(this.keys.actCompare && n_param){  // 重构列 YoYValue YoYInc
  ...
  rowCallback() {
    return (row, rowData) => {
      ...
      $('.' + arrivalRate[0], row).unbind('click').click(() => {
        this.tool.goSupplierRate(this.param, this.back, rowData, this.keys.routerRate[0]);
      });
      $('.' + arrivalRate[1], row).unbind('click').click(() => {
        this.tool.goSupplierRate(this.param, this.back, rowData, this.keys.routerRate[1]);
      });
      $('.' + arrivalRate[2], row).unbind('click').click(() => {
        const _Param = this.indexCompleteService.YoYValueDateSetting(this.param, {
          basData: this.basicDate,
          cakeOne: this.cakeOne,
          basicMonth: this.basicMonth
        });
        this.tool.goSupplierRate(_Param ? _Param : this.param, this.back, rowData, this.keys.routerRate[0]);
      });
      $('.' + arrivalRate[3], row).unbind('click').click(() => {
        ...
        this.tool.goSupplierRate(_Param ? _Param : this.param, this.back, rowData, this.keys.routerRate[1]);
      })
    };
  }
...
angular.module('hs.synthesizeAnalyze').component('supplyItem', {
  templateUrl:  'app/synthesizeAnalyze/directives/component/activity/item/item.tpl.html',
  controller: supplyItemController,
  controllerAs: 'ctrl',
  bindings: {
    param: '<',
    field: '<',
