class ChooseSupplierCtrl {
  constructor(context, $uibModalInstance, $scope, popupToolService, Pop,
    ...
    // 标识当前pop的类型
    this.type = Pop.types.filter(s => s.id === 9)[0];
  ...
  choose(curr, currList) {
    ...
    const key = {
      ...
      multi: this.isMulti,
      content: current,
      maxCount: this.param.MaxLimit ? this.param.MaxLimit : 2000,
    };
  ...
  reverseChoose() {
    ...
    const maxCount = this.param.MaxLimit ? this.param.MaxLimit : 2000;
    const select = this.popupTool.reverseChoose(this.initList, this.selected, this.type, maxCount);
    if (select) this.selected = select;
  }
  ...
  allChoose() {
    const maxCount = this.param.MaxLimit ? this.param.MaxLimit : 2000;
    const select = this.popupTool.allChoose(this.initList, this.selected, this.type, maxCount);
    if (select) this.selected = select;
  }
...
angular.module("hs.popups").controller("supplierCtrl", ChooseSupplierCtrl);
