class StoreGroupController {
  constructor(context, $uibModalInstance, $scope, popupToolService, Pop,
    ...
    // 标识当前pop的类型
    this.type = Pop.types.filter(s => s.id === 8)[0];
  ...
  getList() {
    ...
    this.basic.packager(this.context.promiseFunc(this.buildParam()), res => {
      ...
      this.initList.forEach(s => {
        s.hover = s.stores.map(t => {
          return `[${t.storeCode}]${t.storeName}`
        }).join(',');
      });
...
angular.module('hs.popups').controller('StoreGroupCtrl', StoreGroupController);
