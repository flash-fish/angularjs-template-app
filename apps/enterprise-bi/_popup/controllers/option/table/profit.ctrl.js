class ProfitTableCtrl {
  constructor($uibModalInstance, context, basicService, toolService, CommonCon,
    ...
    this.localOrigin = this.context.local
      ? this.context.local
      : `${CommonCon.local.TABLE_ORIGIN_SUP_PROFIT}_${this.currJob}`;

    // 页面的指标结构
    const basicFields = this.context.basicFields
      ? this.context.basicFields
      : Table.profit;

    this.option = angular.copy(basicFields);
  ...
  init() {
    let local = this.basic.getLocal(this.localOrigin);
    ...
    if (local) {
      ...
      //（既是老板权限又不要tab的时候第四个param要传false => 供应商收益分析）
      this.option = this.tool.getFieldFromLocal(local, this.option, this.currJob, this.isBoss && !this.showTab);
...
angular.module("hs.popups").controller("profitTableCtrl", ProfitTableCtrl);
