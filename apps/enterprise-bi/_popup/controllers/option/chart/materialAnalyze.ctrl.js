class MaterialAnalyzeCtrl {
  constructor(context, $uibModalInstance, Chart, basicService,
    ...
    this.ViewFields = this.context.local
      ? this.context.local.viewFields
      : Chart.goodsAnalyze;
    ...
    this.originField = angular.copy(this.ViewFields);
    ...
    if(this.context.change) {
      _.forIn(this.context.change, (val, key) => {
        ...
        if (_.isEqual(key, "update")) {
          val.list.forEach(s => {list[s[0]][s[1]] = s[2];})
        }
      });
    }
  }
  ...
  change(curr, val, all, tab) {
    ...
    console.log(this.showManyError);
  }
  ...
  ok() {
    ...
    const _conKey = { line:true };

    const fieldSale = this.tool.calculateChartField(chart.sale, type, _conKey);

    const field = {first: fieldSale};
...
angular.module("hs.popups").controller("MaterialAnalyzeCtrl", MaterialAnalyzeCtrl);
