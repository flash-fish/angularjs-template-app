class classPriceCtrl {
  constructor(context, $uibModalInstance, Chart, basicService,
    ...
    this.originField = angular.copy(this.Chart.classBrandChart);
  }
  ...
  ok() {
    ...
    const chart = angular.copy(this.chart);
    const fieldSale = this.tool.calculateChartField(chart.sale);
    const field = {first: fieldSale };

    // 将当前已选中的指标按照一定的结构保存到local中
    this.basic.setLocal(this.localOrigin, { chart: chart});
...
angular.module("hs.popups").controller("classPriceCtrl", classPriceCtrl);
