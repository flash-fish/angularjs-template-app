class newSaleStockChartCtrl {
  constructor(context, $uibModalInstance, Chart, basicService,
    ...
    this.originField = angular.copy(this.Chart.newProductAnalyze_CHART);

    if(this.context.change) {
      _.forIn(this.context.change, (val, key) => {
        const list = this.originField[val.name][val.type].list;
        if (_.isEqual(key, "remove")) { _.remove(list, (n, i) => val.list.includes(i)); }
...
angular.module("hs.popups").controller("newSaleStockChartCtrl", newSaleStockChartCtrl);
