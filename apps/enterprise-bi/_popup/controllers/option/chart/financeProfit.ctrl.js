...
class FinanceProfitChartCtrl {
  constructor($uibModalInstance, Chart, basicService, CommonCon, toolService,
    ...
    this.local = this.context.local
      ? this.context.local
      : {
      origin: `${CommonCon.local.CHART_ORIGIN_SALE_FINANCE_PROFIT}_${this.currJob}`,
      data: `${CommonCon.local.CHART_DATA_SALE_FINANCE_PROFIT}_${this.currJob}`
    };

    // 页面的指标结构
    this.option = angular.copy(Chart.financeAnalyze_profit);

    this.origin = angular.copy(Chart.financeAnalyze_profit);
  ...
  getCurrBar() {
    ...
    if (this.currBar.lineTitle)
      this.currBar.name = this.currBar.lineTitle;
...
angular.module("hs.popups").controller("financeProfitChartCtrl", FinanceProfitChartCtrl);
